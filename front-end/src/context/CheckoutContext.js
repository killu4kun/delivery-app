...
export const CheckoutProvider = ({ children }) => {
  ...
  const productCheckout = (product, quantity) => {
    ...
    if (quantity === 0) {
      const newArray = checkout.filter((productFilter) => productFilter.id !== product.id);
  ...
  const resultTotal = (checkout) => {
    const total = checkout.reduce((total, product) => total + (Number(product.price) * product.quantity), 0)
    return total;
  }
  ...
  return (
    <CheckoutConext.Provider value= { { productCheckout, checkout, totalPrice, setTotalPrice } }>
      { children }
